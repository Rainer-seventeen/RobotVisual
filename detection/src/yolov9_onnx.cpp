...
bool Yolov9Onnx::ReadModel(const std::string &modelPath, bool isCuda, int cudaID, bool warmUp)
{
	if (_batchSize < 1)
		_batchSize = 1;
	try
...
int Yolov9Onnx::Preprocessing(const std::vector<cv::Mat> &srcImgs, std::vector<cv::Mat> &outSrcImgs, std::vector<cv::Vec4d> &params)
{
	outSrcImgs.clear();
...
bool Yolov9Onnx::OnnxDetect(cv::Mat &srcImg, std::vector<OutputParams> &output)
{
 ...
 * @return {*}
 */
bool Yolov9Onnx::OnnxBatchDetect(std::vector<cv::Mat> &srcImgs, std::vector<std::vector<OutputParams>> &output)
{
	vector<Vec4d> params;
