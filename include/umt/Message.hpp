 ...
 * @Date         : 2024-04-07 19:57:34
 * @LastEditors  : Rainer-seventeen
 * @LastEditTime : 2024-04-07 21:04:46
 */
...
namespace umt
{
    ...
    namespace utils
    {
        ...
        public:
            using MsgType = T; // 对外使用MsgType代表信息类型
        private:
            std::mutex pubs_mtx;
            std::list<Publisher<T> *> pubs; // 双向链表
            std::mutex subs_mtx;
    ...
    public:
        ...
        T pop_until(P pt)
        {
            ...
            if (!cv.wait_until(lock, pt, [this]()
                               { return p_msg->pubs.empty() || !fifo.empty(); }))
            // 如果消息队列 fifo 中是否有消息可用，或者消息发布者 p_msg 是否已经停止，wait() 函数将解锁 lock，并等待被唤醒。
            {
                throw MessageError_Timeout();
            }
            if (p_msg->pubs.empty())
